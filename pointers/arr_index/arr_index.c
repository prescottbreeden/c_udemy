 ...
 * =====================================================================================
 *
 *       Filename:  arr_index.c
 *
 *    Description:  calculating any array index
 *
 *        Version:  1.0
 *        Created:  11/19/2018 10:57:48 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
// illustrate the different amounts that are added to 
// an address by C's address arithmetic
int main() {
  int* pa;
  float* pb;
  double* pc;
  ...
  printf("size are: int %d; float %d; long long int %d; short int %d; long int %d\n", 
      sizeof(int), sizeof(float), sizeof(double), sizeof(long long int), 
      sizeof(short int), sizeof(long int));
  // NOTE: because of historical reasons a plain 'int' can vary in size between machine architectures.
  // On some microprocessors it is 2 bytes long while on Intel processors it is 4 bytes
  // if we want to be explicit (and more portable), then use 'long int' for a 32 bit
  // integer instead of just 'int'
